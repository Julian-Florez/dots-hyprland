...
RippleButton {
    ...
    property bool isSurfaceKey: key === "Esc" || key === "Tab" || isBackspace || isEnter
        || key === "Shift" || key === "Ctrl" || key === "Alt" || key === "Menu" || key === "Space"
    ...
    enabled: shape != "empty"
    // Letter, number and symbol keys blend into the keyboard. Only functional
    // keys keep a raised surface; every key retains its hover feedback.
    colBackground: (shape == "empty" || !isSurfaceKey)
