...
AbstractBackgroundWidget {
    ...
    readonly property bool forceCenter: Config.options.background.widgets.clock.forceCenter || (GlobalStates.screenLocked && Config.options.lock.centerClock)
    ...
    readonly property real screenWidthToUse: GlobalStates.screenLocked ? root.screenWidth : root.scaledScreenWidth
    readonly property real screenHeightToUse: GlobalStates.screenLocked ? root.screenHeight : root.scaledScreenHeight
    readonly property real parentOffsetX: (parent && parent.parent) ? parent.parent.x : 0
    readonly property real parentOffsetY: (parent && parent.parent) ? parent.parent.y : 0
    x: forceCenter ? (((screenWidthToUse - root.width) / 2) - parentOffsetX) : targetX
    y: forceCenter ? (((screenHeightToUse - root.height) / 2) - parentOffsetY) : targetY
