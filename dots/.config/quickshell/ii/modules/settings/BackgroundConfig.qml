...
ContentPage {
    ...
    ContentSection {
        ...
        ConfigSwitch {
            ...
            text: Translation.tr("Always center clock")
            checked: Config.options.background.widgets.clock.forceCenter
            onCheckedChanged: {
                Config.options.background.widgets.clock.forceCenter = checked;
            }
        }
        ...
        ContentSubsection {
            ...
            ConfigSlider {
                ...
                value: Config.options.background.widgets.clock.cookie.size
                usePercentTooltip: false
                buttonIcon: "photo_size_select_large"
                from: 50
                to: 800
                stopIndicatorValues: [230]
                onValueChanged: {
                    Config.options.background.widgets.clock.cookie.size = value;
                }
            }
        ...
        ContentSubsection {
            visible: settingsClock.cookiePresent
            title: Translation.tr("Cookie clock colors")

            ConfigSwitch {
                buttonIcon: "palette"
                text: Translation.tr("Enable custom colors")
                checked: Config.options.background.widgets.clock.cookie.customColors
                onCheckedChanged: {
                    Config.options.background.widgets.clock.cookie.customColors = checked;
                }
            }

            ColumnLayout {
                visible: Config.options.background.widgets.clock.cookie.customColors
                spacing: 12
                Layout.fillWidth: true

                ColorField {
                    label: Translation.tr("Background color")
                    text: Config.options.background.widgets.clock.cookie.colBackground
                    onTextChanged: {
                        Config.options.background.widgets.clock.cookie.colBackground = text;
                    }
                }

                ColorField {
                    label: Translation.tr("Dial / Marks color")
                    text: Config.options.background.widgets.clock.cookie.colOnBackground
                    onTextChanged: {
                        Config.options.background.widgets.clock.cookie.colOnBackground = text;
                    }
                }

                ColorField {
                    label: Translation.tr("Date / Quote color")
                    text: Config.options.background.widgets.clock.cookie.colBackgroundInfo
                    onTextChanged: {
                        Config.options.background.widgets.clock.cookie.colBackgroundInfo = text;
                    }
                }

                ColorField {
                    label: Translation.tr("Hour hand color")
                    text: Config.options.background.widgets.clock.cookie.colHourHand
                    onTextChanged: {
                        Config.options.background.widgets.clock.cookie.colHourHand = text;
                    }
                }

                ColorField {
                    label: Translation.tr("Minute hand color")
                    text: Config.options.background.widgets.clock.cookie.colMinuteHand
                    onTextChanged: {
                        Config.options.background.widgets.clock.cookie.colMinuteHand = text;
                    }
                }

                ColorField {
                    label: Translation.tr("Second hand color")
                    text: Config.options.background.widgets.clock.cookie.colSecondHand
                    onTextChanged: {
                        Config.options.background.widgets.clock.cookie.colSecondHand = text;
                    }
                }
            }
        }
    ...
    component ColorField: RowLayout {
        ...
        property color previewColor: textField.text
        spacing: 12
        Layout.fillWidth: true
        ...
        Rectangle {
            ...
            height: 32
            radius: 6
            color: colorFieldRoot.previewColor
        ...
        MaterialTextField {
            id: textField
            Layout.fillWidth: true
            placeholderText: colorFieldRoot.label
        }
    }
}
