...
Scope { // Scope
    ...
    property real verticalOffset: Config.options?.osk.verticalOffset ?? 0
        ...
        sourceComponent: PanelWindow { // Window
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    spacing: 5
                    Item {
                        implicitWidth: oskControls.implicitWidth
                        implicitHeight: oskControls.implicitHeight
                        ...
                        // Drag the narrow left edge to raise or lower the keyboard.
                        MouseArea {
                            ...
                            onPressed: event => {
                                startOffset = root.verticalOffset;
                                pressY = event.y;
                            }
                            onPositionChanged: event => {
                                if (!pressed) return;
                                const maxOffset = Math.max(0, oskRoot.screen.height - oskRoot.implicitHeight);
                                root.verticalOffset = Math.max(0, Math.min(maxOffset, startOffset - (event.y - pressY)));
                            }
                            onReleased: Config.options.osk.verticalOffset = root.verticalOffset
                        }
                    }
                    Rectangle {
                        Layout.topMargin: 20
