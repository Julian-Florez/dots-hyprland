...
Singleton {
    ...
    function processNotificationBody(body, appName) {
        ...
        // KDE Connect encodes line breaks as HTML tags. Normalize them before
        // the notification view chooses whether to render plain or rich text.
        processedBody = processedBody.replace(/&lt;br\s*\/?\s*&gt;/gi, '\n');
        processedBody = processedBody.replace(/<br\s*\/?\s*>/gi, '\n');
    ...
    function isKdeConnect(appName) {
        return (appName || "").toLowerCase().includes("kde connect")
    }

    function shouldRenderBodyAsPlainText(body, appName) {
        return isKdeConnect(appName) || /<br\s*\/?\s*>/i.test(body || "")
    }
}
